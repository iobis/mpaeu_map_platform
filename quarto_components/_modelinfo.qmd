...
masks_options <- c("fit_region_max_depth", "native_ecoregions", "fit_region", "convex_hull", "minbounding_circle", "buffer100m")
names(masks_options) <- c("Fit area (max. depth)", "Native Realms", "Fit area", "Convex Hull", "Min. bounding circle", "Buffer 100m")

conditionalPanel(
  condition = "input.speciesSelect !== '' && (input.jsValue?.id == null || input.jsValue?.id === 'species')",
  tags$div(
...
conditionalPanel(
  condition = "input.speciesSelectThermal !== '' && input.jsValue?.id === 'thermal'",
  tags$div(
      #shiny::selectInput("ecspMask", label_select_mask, masks_options, width = "360px"),
      bslib::input_switch("ecspRealms", label_switch_realms, FALSE),
      bslib::input_switch("ecspEEZ", label_switch_eez, FALSE),
...
conditionalPanel(
  condition = "input.habitatSelect !== '' && input.jsValue?.id === 'habitat'",
  tags$div(
      bslib::input_switch("ecspRealms", label_switch_realms, FALSE),
      bslib::input_switch("ecspEEZ", label_switch_eez, FALSE),
      bslib::input_switch("ecspMPA", label_switch_mpa, FALSE),
...
conditionalPanel(
  condition = "input.diversitySelect !== '' && input.jsValue?.id === 'diversity'",
  tags$div(
      bslib::input_switch("ecspRealms", label_switch_realms, FALSE),
      bslib::input_switch("ecspEEZ", label_switch_eez, FALSE),
      bslib::input_switch("ecspMPA", label_switch_mpa, FALSE),
...
<!-- Table 1 --->
::: {.g-col-md-6 .g-col-12 data-display-if="!input.jsValue || !(input.jsValue.id == 'habitat' || input.jsValue.id == 'diversity' || input.jsValue.id == 'atlas')"}
`r tags$div(id="table1-title", class="body-title", textOutput("tableATitle"))`

```{r}
reactable::reactableOutput("tableA")
```
...
```{r}
reactable::reactableOutput("tableB")
```
...
::: {.g-col-md-8 .g-col-12}
```{r}
scenarios_dec <- paste(rep(paste0("ssp", c(126, 245, 370, 460, 585)), each = 2), c("dec50", "dec100"), sep = "_")
names(scenarios_dec) <- paste(rep(paste0("SSP", 1:5), each = 2), c("2050", "2100"), sep = " ")
scenarios_dec <- c("Current" = "current", scenarios_dec)
conditionalPanel(
          condition = "input.extraButton != 0 && input.extraButton % 2 != 0 && (input.jsValue?.id ?? 'species') === 'species'",
          tags$div(selectInput("extraPlotBSelect", "Scenario and period", choices = scenarios_dec),
                   style = "font-size: smaller; width: min-content; justify-self: center;"),

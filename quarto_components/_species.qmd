...
# Add selectors ----
# Group selector
avg <- yaml::read_yaml("data/sdm_conf.yml", readLines.warn = F)
avg <- names(avg$groups)
group_options <- c("all", avg)
names(group_options) <- stringr::str_to_title(group_options)
selectInput(
   inputId = "groupSelect",
   label = "Select group", 
